...
bool CAgramtab :: Read (const char * FileName) 
{
	...
	//	return false;
        ...
        Stroka fileContent = GetMorphFileByName(Stroka(FileName));
        if (0 == fileContent.size())
          return false;
        //VectorStrok lines = splitStroku(fileContent, "\r\n");
        istringstream ss(fileContent.c_str());

	char buff[300];
	size_t LineNo = 0;
	while (ss.getline(buff, 300)) //fgets (buff,300,fp))
	//for(size_t LineNo = 0; LineNo < lines.size(); LineNo++)
	{
		const char * s = buff;
		for (; isspace ( (const unsigned char) *s); s++);
		if (!*s) continue;
...
bool CAgramtab::Load(std::string path)
{
    try
    {
        Read(path.c_str());
...
bool CAgramtab::LoadAndCheck (std::string path)
{
    try
    {
        return ReadAndCheck(path.c_str());
    }
    catch (...)
    {
        return false;
