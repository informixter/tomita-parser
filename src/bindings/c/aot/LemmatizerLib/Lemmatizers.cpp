...
bool CLemmatizer::LoadDictionaries(const string& dicPath, string& strError) {
    string load_path = dicPath;
    if ((load_path.length() > 0)
        && (load_path[load_path.length() - 1] != '\\')
        && (load_path[load_path.length() - 1] != '/'))
        load_path += "/";
...
// TODO: после конвертации в UTF-8 этот код перестал рабоать. Нужно переделывать!
