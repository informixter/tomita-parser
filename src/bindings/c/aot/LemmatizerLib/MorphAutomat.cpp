...
bool CMorphAutomat::Load(string AutomatFileName)
{
	...
	TInputStream *pInput = GetObjectByName(Stroka(AutomatFileName));
	if (NULL == pInput)
	...
	Stroka t = pInput->ReadLine();
	m_NodesCount = atoi(t.c_str());
	...
	//	return  false;
	if (sizeof(CMorphAutomNode) * m_NodesCount != pInput->Read(m_pNodes, sizeof(CMorphAutomNode) * m_NodesCount))
	    return false;
	

	//if (!fgets(buffer, 256, fp)) return false;
	t = pInput->ReadLine();
	m_RelationsCount = atoi(t.c_str());
	...
	//	return  false;
	if (sizeof(CMorphAutomRelation) * m_RelationsCount != pInput->Read(m_pRelations, sizeof(CMorphAutomRelation) * m_RelationsCount))
	    return false;

	{
		int Alphabet2Code[256];
		//fread(Alphabet2Code,sizeof(int),256,fp);
		pInput->Read(Alphabet2Code, sizeof(int) * 256);
