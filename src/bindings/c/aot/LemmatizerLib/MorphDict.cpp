...
bool CMorphDict::Load(string GrammarFileName)
{
	...
	TInputStream *pInput = GetObjectByName(Stroka(PrecompiledFile));
	if (NULL == pInput)
	{
...
#else
	ReadFlexiaModels(pInput, m_FlexiaModels);

	ReadAccentModels(pInput, m_AccentModels);
	...
	m_LemmaInfos.clear();
	ReadVectorInner(pInput, m_LemmaInfos, Count);
	...
	m_NPSs.clear();
	ReadVectorInner(pInput, m_NPSs, Count);
