...
void ReadFlexiaModels(TInputStream* pInput, vector<CFlexiaModel>& FlexiaModels )
{
#ifdef BOOST
	...
	int paradigm_count = atoi(t.c_str());
...
void ReadAccentModels (TInputStream* pInput, vector<CAccentModel>& AccentModels )
{
#ifdef BOOST
	...
	for(int b = 0; b < count; b++)
	{
		...
		CAccentModel M;
		if (!M.ReadFromString(t.c_str()))
			throw CExpc("Cannot parse line %s", t.c_str());
...
{
	throw CExpc("MorphoWizard::load_mrd commented out");
