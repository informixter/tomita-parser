...
bool IsSuperEqualChar (BYTE ch1, BYTE ch2, MorphLanguageEnum langua)
{
	...
	char const EF[LenEF+1] = "\0\xE0\xC0aA\0\xE5\xC5Ee\0\xE7\xC73\0\xEA\xCAKk\0\xEC\xCCM\0\xED\xCDH\0\xEE\xCEOo0\0\xF0\xD0pP\0\xF1\xD1Cc\0\xF3\xD3y\0\xF5\xD5Xx";
...
BYTE force_rus_char (BYTE ch)
{
	...
	// во второй строке находятся русские: АаЕеЗКкМНОоРрСсуХхиТ
	char replace[] = "\0xC0\0xE0 \0xC5\0xE5 0xC7 \0xCA\0xEA \0xCC \0xCD \0xCE\0xEE \0xD0\0xF0 \0xD1\xF1 \0xF3 \0xD5\0xF5 \0xE8 \0xD2";
...
{
	clear();
	file_off_t BufferSize = (int)GetObjectSizeByName(Stroka(filename)) - sizeof(size_t);

	//FILE* fp = fopen(filename.c_str(), "rb");
	TInputStream *pInput = GetObjectByName(Stroka(filename));

	if (NULL == pInput) return;
	//if (!fp) return;
	size_t Count;
	pInput->Read((void*)&Count, sizeof(size_t));
	//fread ((void *)&Count, 1, sizeof(size_t), fp);
	try {
		m_Buffer.clear();
		ReadVectorInner(pInput, m_Buffer, BufferSize);
		//fclose(fp);
		//fp = 0;
	}
	catch (...)
