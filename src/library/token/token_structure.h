...
struct TCharSpan {
    ...
    TCharSpan(size_t offset, ui16 len, ETokenType type = TOKEN_WORD, ETokenDelim tokenDelim = TOKDELIM_NULL, EHyphenType hyphen = HYPHEN_NONE,
...
//        Tokens.reserve(MAX_SUBTOKENS); don't reserve memory
        ...
        ui16 suffixLen = 0, ui16 prefixLen = 0)
    {
        ...
        if (Tokens.size() == MAX_SUBTOKENS) {
            ...
            // insert a new one
            Tokens.push_back(TCharSpan()); // yresize() ?
            TCharSpan& span = Tokens.back();
            span.Pos = offset;
            span.Len = len;
            span.PrefixLen = prefixLen;
            span.SuffixLen = suffixLen;
            span.Type = type;
            span.Hyphen = hyphen;
            span.TokenDelim = tokenDelim;
        }
    }
    void pop_back() {
        Tokens.pop_back();
...
struct TWideToken
{
    ...
    TWideToken(const TChar* s, size_t len)
        : Token(s)
        , Leng(len)
    {
    ...
    TWideToken(const TChar* token, size_t len, const TTokenStructure& subTokens)
        : Token(token), Leng(len), SubTokens(subTokens)
    {
    }

    void Concat(const TWideToken& tok) {
        YASSERT(Token && Leng && !SubTokens.empty() && Token + Leng <= tok.Token);
    ...
    TWideToken Extract(size_t begin, size_t end) const {
        ...
        YASSERT(ret.Leng + shift <= Leng);
        return ret;
    }
};

inline bool operator == (const TWideToken& a, const TWideToken& b) {
    if ((a.Leng != b.Leng) || memcmp(a.Token, b.Token, a.Leng * sizeof(TChar)))
        return false;
